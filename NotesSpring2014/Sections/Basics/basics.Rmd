...
An advantage of the conventions for dataframes is that it is very easy to access data using standard computer software.  Store the dataframe in a "file", read in the file and refer to it by a name you give it.  Often, you will be dealing with different dataframes at the same time; the ability to refer to one or another by name is important.


This paradigm means that you can carry out data analysis by knowing just a handful of operations.  Some of these operations convert one or more dataframes to a new form: what has been called **domesticating data**. Other operations generate the presentation from whatever dataframe is provided.
