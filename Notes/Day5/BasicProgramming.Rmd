...
Consider the Gapminder data.  It's organized as one physical variable per file, with a case identifier involving two variables: Country and Year.
...
* That you see enough structure that you could, for example, change the subset of years being considered, or shift from, say, the `mean` to the `max` or some other operation.  

You can copy and paste the commands into your own document in order to carry them out.

### A Parallel Axes Plot

Notes for a potential exercise.  What's the relationship between alcohol consumption and traffic daths.  It appears that fertility has a stronger connection --- the higher the per capita alcohol consumption, the lower the traffic death rate.  Ecological paradox: that this pattern holds true for countries doesn't mean that it will hold true for individual people.
