...
```{r}
require(mclust)
clusters <- Mclust(getVarFormula(~MarriageAgeFemale+ContraceptiveUse+Fertility+ PrimaryEdCompletionFemale, data=ress  ))
clusterID = whichCluster(clusters)
together = cbind(ress, clusterID)
{.s. <- joinCountryData2Map(together, joinCode='NAME',nameJoinColumn='Country' ); par(mai=c(0,0,0.2,0), xaxs='i',yaxs='i') ;.mp. <- mapCountryData(.s., nameColumnToPlot='Cluster',catMethod='categorical',mapRegion='world')} 
```
